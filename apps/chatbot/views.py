...
def chat_stream(request):
    ...
    def event_stream():
        ...
        if confidential_hit(retrieved):
            escalation = _auto_escalate_confidential(
                request=request,
                conversation=conversation,
                question=query,
                bot_answer=CONFIDENTIAL_REFUSAL_MESSAGE,
            ...
            yield _sse({
                ...
                'escalate': True,
                'auto_escalated': True,
                'email_status': escalation['email_status'],
            })
...
def _format_ask_hr_email(record):
    """Build a concise, professional plain-text escalation email."""
    convo_ref = f"Conversation #{record.conversation_id}" if record.conversation_id else "Not available"
    created_local = timezone.localtime(record.created_at)
    divider = '-' * 72

    lines = [
        'STEMZ HR ASSISTANT - AUTOMATIC HR ESCALATION',
        divider,
        '',
        'An employee asked a question that requires confidential HR review.',
        'The assistant did not disclose the protected policy information.',
        "",
        'EMPLOYEE DETAILS',
        divider,
        f"Name:        {record.user_name or 'Not provided'}",
        f"Email:       {record.user_email or 'Not provided'}",
        f"Time:        {created_local.strftime('%d %b %Y, %H:%M')} IST",
        f"Reference:   {convo_ref}",
        "",
        'QUESTION REQUIRING HR REVIEW',
        divider,
    ...
    lines += [
        'ESCALATION REASON',
        divider,
        record.user_note or 'Confidential policy question detected.',
        '',
    ]
    ...
    if record.transcript:
        ...
        for turn in record.transcript[-8:]:
    ...
    lines += [
        ...
        "ACTION REQUESTED",
        divider,
        "Please review the question and reply directly to the employee's email address.",
        "This notification was generated automatically by the Stemz HR Assistant.",
    ]
    return '\n'.join(lines)


def _auto_escalate_confidential(request, conversation, question, bot_answer, retrieved_titles, top_score):
    """Notify HR immediately when a confidential policy question is detected."""
    user = request.user
    name = (user.get_full_name() or user.get_username() or user.email or 'Employee').strip()[:120]
    ...
    record = AskHRRequest.objects.create(
        ...
        user_note='Automatic escalation: confidential policy question detected.',
    ...
    try:
        ...
        message = EmailMessage(
            subject=f'[HR Bot Confidential Escalation] {name}: {question[:60]}',
            body=_format_ask_hr_email(record),
            from_email=settings.DEFAULT_FROM_EMAIL,
            to=recipients,
    ...
    return {'email_status': record.email_status}
